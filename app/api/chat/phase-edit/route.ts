...
배열 항목 추가/삭제 — 특수 연산자 사용:
7. 배열에 새 항목을 추가할 때는 $append 연산자를 사용하세요:
   예: { "targetPersonas": { "$append": [{ "name": "신규 페르소나", ... }] } }
8. 배열에서 항목을 삭제할 때는 $remove 연산자(0-기반 인덱스 배열)를 사용하세요:
   예: { "wbsTasks": { "$remove": [2] } }  // 2번 인덱스(3번째) 항목 삭제
9. $append/$remove는 기존 배열과 합산되므로, 기존 데이터를 재입력할 필요가 없습니다.`
